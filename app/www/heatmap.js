...
function rebuildColors() {
  // recompute all cell colors with current palette
...
function buildHeatmap(data) {
  ...
  if (isMini) {
    ...
    // dynamic top offset based on actual header height
...
function buildMiniView(container, eps, lookup, maxSeason, maxEp, maxRank) {
  ...
  // update fade visibility on scroll and on initial render
  ...
  // defer initial check so layout is computed
...
function showTooltip(e) {
  if (_isTouch) return; // handled by handleCellTap on touch
  clearTimeout(_hideTooltipTimer);
  _activeTooltipCell = e.currentTarget;
  populateTooltip(e.currentTarget.dataset);
  var tip = document.getElementById("tooltip");
  // position above the cell, not at cursor, so it's reachable
  var rect = e.currentTarget.getBoundingClientRect();
  tip.style.left = rect.left + "px";
  tip.style.top = (window.scrollY + rect.top - tip.offsetHeight - 6) + "px";
  tip.style.display = "block";
  // reposition after render if it was hidden (offsetHeight was 0)
  tip.style.top = (window.scrollY + rect.top - tip.offsetHeight - 6) + "px";
}

function moveTooltip(e) {
  // no-op: tooltip is positioned relative to cell, not cursor
}
...
// message handler from Shiny
Shiny.addCustomMessageHandler("showDetail", function(msg) {
  var data = typeof msg === "string" ? JSON.parse(msg) : msg;
  // compute year range
  var years = data.episodes.map(function(e) { return e.year; }).filter(function(y) { return y; });
  var yearRange = "";
  if (years.length > 0) {
  ...
  document.getElementById("detail-title").textContent = data.title + yearRange;
...
document.addEventListener("DOMContentLoaded", function() {
  // tooltip: hover to keep it for clicking links, tap elsewhere to dismiss
  var tip = document.getElementById("tooltip");
